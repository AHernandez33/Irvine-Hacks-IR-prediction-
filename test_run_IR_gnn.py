...
NUM_FILTERS   = 128
NUM_INTER     = 3        # 2 interaction blocks is enough for small datasets
BATCH_SIZE    = 16
MAX_EPOCHS    = 600
LR            = 1e-3
...
def smiles_to_data(smiles, y=None):
    ...
    if result != 0:
        ...
        result = AllChem.EmbedMolecule(mol, randomSeed=None)
    if result != 0:
        return None
...
class IRDataset(Dataset):
    ...
    Automatically removes extra/unnamed columns.
    """
    ...
    def __init__(self, csv_path):
        ...
        # Detect name and SMILES columns flexibly
        name_col   = next((c for c in cols if 'name' in c.lower()), cols[0])
        smiles_col = next((c for c in cols if 'smile' in c.lower()), cols[1])

        # All remaining columns are transmittance points
        point_cols = [c for c in cols if c not in (name_col, smiles_col)]

        # If more than N_POINTS, keep only the first N_POINTS
        if len(point_cols) > N_POINTS:
            print(f"[!] Found {len(point_cols)} spectral columns — keeping first {N_POINTS}")
            point_cols = point_cols[:N_POINTS]

        # If fewer than N_POINTS, raise an error
        if len(point_cols) < N_POINTS:
            raise ValueError(
        ...
        print(f"Dataset loaded: {len(self.samples)} molecules ({skipped} skipped due to 3D failure)")
